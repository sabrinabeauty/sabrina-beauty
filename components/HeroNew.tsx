...
export default async function HeroNew() {
  ...
  return (
    <section className="relative h-[calc(100dvh-80px)] max-h-[740px] min-h-[560px] sm:h-[80vh] sm:max-h-none sm:min-h-[560px] flex items-end sm:items-center bg-cream">
      <Image
      ...
      {/* On mobile the full photo (face included) is visible top-to-bottom, so the text card
          sits low, over her chest/shoulder area, keeping her face and smile uncovered above it.
          100dvh (not vh) accounts for Safari's collapsing toolbar so this holds on a real phone,
          not just a fixed-height headless browser; max-h caps it on very tall/no-toolbar views. */}
      <div className="relative w-full max-w-6xl mx-auto px-6 pb-8 sm:pb-0">
        <div className="max-w-md ml-auto text-right bg-cream/85 backdrop-blur-md rounded-xl2 px-5 py-5 sm:bg-transparent sm:backdrop-blur-none sm:px-0 sm:py-0">
          <h1 className="font-serif text-3xl sm:text-6xl mb-3 sm:mb-6 text-charcoal [text-shadow:0_1px_12px_rgba(246,210,210,0.8)] sm:[text-shadow:none]">
            {heroHeadline}
          </h1>
          <p className="text-sm sm:text-xl mb-4 sm:mb-8 text-charcoal/80">{heroTagline}</p>
          <Link
            href="/book"
